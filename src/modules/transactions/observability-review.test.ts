...
import {
  classifyTransactionReviewError,
  createTransactionReviewOperation,
  getTransactionReviewSlowQueryThresholdMs,
  isExpectedTransactionReviewError,
  logTransactionReviewOperation,
  measureTransactionReviewQuery,
  sanitizeTransactionReviewLog,
  toTransactionReviewErrorEnvelope,
...
describe("transaction review observability", () => {
  it("creates the four code-owned operation/query identifiers and an opaque request id", () => {
    const operation = createTransactionReviewOperation("list", {
  ...
  it("allow-lists aggregate metadata and drops every prohibited S05 field", () => {
    ...
    expect(safe).toEqual({
      event: "transaction_review_detail_success",
  ...
  it("classifies only ADR/S03 expected codes and never uses an exception message", () => {
    for (const error of [
      ...
      expect(classifyTransactionReviewError(error)).toMatchObject({
        outcome: "expected_error",
      });
      expect(isExpectedTransactionReviewError(error)).toBe(true);
    }

    const technical = new Error(
      "database failure search=private description=private amount=999999",
    );
    expect(classifyTransactionReviewError(technical)).toEqual({
      outcome: "unexpected_error",
      errorCode: "UNEXPECTED_ERROR",
    });
    expect(isExpectedTransactionReviewError(technical)).toBe(false);
    expect(classifyTransactionReviewError({ code: "NOT_AN_EXPECTED_CODE" })).toEqual({
      outcome: "unexpected_error",
      errorCode: "UNEXPECTED_ERROR",
    });
  });
  ...
  it("returns a code-only safe error envelope", () => {
    expect(toTransactionReviewErrorEnvelope({ code: "INVALID_QUERY", message: "search=private" })).toEqual({
      ok: false,
      error: { code: "INVALID_QUERY" },
    });
    expect(toTransactionReviewErrorEnvelope(new Error("private technical message"))).toEqual({
      ok: false,
  ...
  it("logs expected Result failures without capturing them in Sentry", async () => {
    ...
    const operation = createTransactionReviewOperation("summary", {
      requestId: "request-opaque",
    });
    const result = {
      ok: false as const,
    ...
    await expect(
      withTransactionReviewObservability(operation, () => result, {
        now,
        needsReviewCount: 0,
      }),
  ...
  it("captures only unexpected technical failures with sanitized context", async () => {
    ...
    const operation = createTransactionReviewOperation("update", {
      requestId: "request-opaque",
      eventId: "event-opaque",
      householdId: "household-opaque",
      origin: "IMPORT",
  ...
  it("measures slow queries above the configured threshold and preserves results", async () => {
    ...
    await expect(
      measureTransactionReviewQuery(operation, () => "read-result", {
        thresholdMs: 250,
        now,
        pageSize: 50,
        resultCount: 50,
  ...
  it("does not emit a slow-query record below threshold and keeps expected errors non-Sentry", async () => {
    ...
    const operation = createTransactionReviewOperation("detail", {
      requestId: "request-opaque",
    });

    await expect(
      measureTransactionReviewQuery(operation, () => "fast-result", {
        thresholdMs: 250,
        now: fastNow,
        onSlowQuery,
      }),
  ...
  it("bounds and reads the configurable threshold", () => {
    expect(getTransactionReviewSlowQueryThresholdMs("900")).toBe(900);
    expect(getTransactionReviewSlowQueryThresholdMs(0)).toBe(0);
    expect(getTransactionReviewSlowQueryThresholdMs("999999")).toBe(60_000);
    expect(getTransactionReviewSlowQueryThresholdMs("not-a-number")).toBe(250);
    expect(getTransactionReviewSlowQueryThresholdMs(-10)).toBe(0);
  });
  ...
  it("emits a safe allow-listed record and breadcrumb for direct integrations", () => {
    ...
    const record = logTransactionReviewOperation(operation, "success", {
      durationMs: 12,
      pageSize: 2,
      resultCount: 2,
    ...
    expect(record).toMatchObject({
      event: "transaction_review_list_success",
